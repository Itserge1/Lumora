...
def is_valid_json(output_data):
    """Test that output is valid json"""
    try:
        json.dumps(output_data)
        return True
    except Exception as e:
        print(f"Output is not a valid JSON: {e}")
        return False

def has_valid_metadata(output_data):
    """Test that output Json have required metadata"""
    require_top_metadata = [
        "patient_name",
        "date_of_birth",
        "report_date",
        "ordering_physician",
    ...
    for metadata in require_top_metadata:
...
# TEST STRUCTURAL ACCURACY OF THE JSON OUTPUT
